...
public class BendecUtils {
    ...
    public static byte[] stringToByteArray(String value, int length) {
        ...
        for (int i = 0; i < value.length(); i++) {
            char charValue = value.charAt(value.length() - i - 1);
    ...
    public static String stringFromByteArray(byte[] bytes, int offset, int length) {
        ...
        for (int i = 0; i < chars.length; i++) {
            chars[i] |=  ((int) bytes[offset + (length - i - 1)]);
            if(((int) bytes[offset + (length - i - 1)]) == 0) {
                stringLength = stringLength - 1;
            }
        }
        return new String(Arrays.copyOfRange(chars, length - stringLength, length));
    }
